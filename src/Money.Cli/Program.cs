...
internal class Program
{
        ...
        Func<TParam, Lazy<ServiceProvider>, IServiceCollection, Task> action)
    {
        ...
        // var defaultLogger = serviceProviderLazy.Value.GetRequiredService<ILogger<Program>>();
    ...
    private static async Task _HandleFreedomFinanceBank(
        ...
        IServiceCollection serviceCollection)
    {
        var logger = serviceProvider.Value.GetRequiredService<ILogger<FreedomFinanceBankParameters>>();
        switch (parameters.ConvertTo)
        {
            case DataSourceType.ZenMoney:
                ...
                foreach (var transaction in ZenMoneyConverter.ConvertToTransactions(ffinTransactions))
                {
    ...
    private static Task _HandleZenMoney(
        ...
        IServiceCollection serviceCollection)
    {
        serviceCollection.PostConfigure<IZenMoneyConfiguration>(
            options =>
                {
                    options.LogAndUpdatePropertiesOf(
                        parameters,
