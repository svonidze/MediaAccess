...
def fetch_transactions_full(account_id=None, type_notlike="uit", limit=21):
    """
    Fetch all transactions from the ZenMoney API by paginating through results.
    Returns a complete list of all transactions that match the query.
    """
    skip = 0
    all_transactions = []
    ...
    while True:
        url = (
            f"{API_URL}/transaction/?limit={limit}&skip={skip}&type_notlike={type_notlike}&finder="
        )
        if account_id:
            url += f"&account%5B%5D={account_id}"
        ...
        if len(batch) < limit:
            break
        skip += limit
    print(f"Total fetched transactions: {len(all_transactions)}")
...
if __name__ == "__main__":
    # Example: Fetch and print all transactions for a specific account
    all_transactions = fetch_transactions_full(account_id=14491595)
    print(f"All transactions (sample): {all_transactions[:3]}")  # Print a sample
